        ...
        for _ in range(iterations):
            for i in range(5):
                for j in range(i+1,5):
                    ...
                    (dx, dy, dz) = (x1-x2, y1-y2, z1-z2)
                    #update_vs(v1, v2, dt, dx, dy, dz, m1, m2)
                    v1[0]-=dx*m2*0.01 * ((dx * dx + dy * dy + dz * dz)**(-1.5))
                    v1[1]-=dy*m2*0.01 * ((dx * dx + dy * dy + dz * dz)**(-1.5))
                    v1[2]-=dz*m2*0.01 * ((dx * dx + dy * dy + dz * dz)**(-1.5))
                    v2[0]+=dx*m1*0.01 * ((dx * dx + dy * dy + dz * dz)**(-1.5))
                    v2[1]+=dy*m1*0.01 * ((dx * dx + dy * dy + dz * dz)**(-1.5))
                    v2[2]+=dz*m1*0.01 * ((dx * dx + dy * dy + dz * dz)**(-1.5))
            for body in BODIES.keys():
                [r,[vx, vy, vz],m]= BODIES[body]
                #update_rs(r, dt, vx, vy, vz)
                r[0] += 0.01 * vx
